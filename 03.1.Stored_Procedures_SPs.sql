...
-- Create the stored procedure dbo.cuspSumRideHrsSingleDay
...
-------------------------------------------------------------------------------
-- check if the procedure is exist
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'cusp_RideSummaryCreate')  
  DROP PROCEDURE cusp_RideSummaryCreate  
go

-- Create the stored procedure to insert a record into the RideSummary table.
CREATE PROCEDURE dbo.cusp_RideSummaryCreate (@DateParm date, @RideHrsParm numeric)
AS
BEGIN
  SET NOCOUNT ON
